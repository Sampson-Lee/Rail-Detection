...
val_loader, _ = get_train_loader(1, data_root, griding_num=100, distributed=False, num_lanes=4, mode='val', type=type)
...
for i, data_label in enumerate(tqdm.tqdm(val_loader)):
    # if i==5: break
    _, _, inter_labels, names = data_label
    ...
    # plt.imshow(img_draw)
    # plt.pause(2)
    vis_path = os.path.join(data_root, 'hand-crafted/vis', names[0]).replace('pic', type)
    if not os.path.exists(os.path.dirname(vis_path)): os.makedirs(os.path.dirname(vis_path))
    cv2.imwrite(vis_path, img_draw)
    pred_path = os.path.join(data_root, 'hand-crafted/pred', names[0]).replace('pic', type)
